...
// VU tuning
#define THRESH_LOW   20000 // adjust here for comp 30,000
#define THRESH_HIGH  200000 // adjust here for comp keep at 200,000? 
...
#define IS_ODD_SIGN      true //true for 1 and 8
...
// ESP-NOW: ask peers what mode they're currently in (used after waking)
...
// ESP-NOW: called when this board receives a message/mode
...
// Effect #6: AlternatingRed
void effectAlternatingRed() {
  if (millis() - lastAltSwitch >= ALT_INTERVAL_MS) {
    lastAltSwitch = millis();
    altState ^= 1;
  }

  bool showRed = IS_ODD_SIGN ? (altState == 0) : (altState == 1);
...
void effectAlternatingBlue() {
  if (millis() - lastAltSwitch >= ALT_INTERVAL_MS) {
    lastAltSwitch = millis();
    altState ^= 1;
  }

  bool showBlue = IS_ODD_SIGN ? (altState == 0) : (altState == 1);
...
void setup() {
  ...
  // If we just woke from deep sleep, this reconnects and matches whatever
  // mode the other signs are currently showing.
  requestModeFromPeers();
  Serial.println("Requested current mode from peers");

  // In case no peer replies, start the battery-saving timeout window now.
...
void loop() {
  ...
  if (btnState == LOW && lastButtonState == HIGH) {
    ...
    Serial.printf("Mode → %s\n",
      ...
      currentMode == 4 ? "Solid Blue" : 
      currentMode == 5 ? "Solid Red" :
  ...
  switch (currentMode) {
    ...
    case 7: effectAlternatingBlue(); break;
  }
